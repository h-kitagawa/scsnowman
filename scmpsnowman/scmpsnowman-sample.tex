\iftrue % true if use LuaTeX-ja
  \documentclass[twocolumn]{ltjsarticle}
  \usepackage{lltjext}
\else
  \documentclass[twocolumn]{article}
  \usepackage{fontspec}\setmainfont{ipaexm.ttf}\setsansfont{ipaexg.ttf}\def\zw{em}
\fi
\usepackage[margin=21truemm]{geometry}
\usepackage[svgnames]{xcolor}
\usepackage{scmpsnowman}
\title{\textsf{scmpsnowman}パッケージの実用例}
\begin{document}
\maketitle
...
\textbf{周囲の色に追随するのは\textsf{xcolor}パッケージが読み込まれている
  場合のみです（\textsf{scmpsnowman}パッケージは\textsf{color}パッケージのみを
  標準で読み込みます）．}
...
ゆきだるま：
\scsnowman[scale=1,hat,snow]%
\scsnowman[scale=3,hat,snow]%
\scsnowman[scale=5,snow,hat]%


枠線付き：
{\fboxsep=0pt
\fbox{\scsnowman[scale=1,hat,snow]}%
\fbox{\scsnowman[scale=3,hat,snow]}%
\fbox{\scsnowman[scale=5,hat,snow]}%
}

\centering
\scsnowman[scale=8,note=blue,hat=green!50!black,
  snow=cyan,muffler=red,buttons=cyan,arms=brown,sweat=white]
